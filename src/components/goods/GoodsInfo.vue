...
export default {
	...
	methods:{
		...
		// 			//因为轮播图组件中只认识item.img，不认识item.src，所以循环数组的每一项，使其item.img=item.src
		...
		goDesc(id) {  //点击使用编程式导航跳到介绍页面
          this.$router.push({ name: "goodsdesc" ,params: {id} })
		},
        goComment(id) { //点击跳到评论页面
        	this.$router.push({ name: "goodscomment", params: {id} })
        },
        addToShopCar() { //将数量添加到购物车
        ...
        enter(el,done) {
        	...
        	el.style.transform = `translate(${xDist + 20}px, ${yDist}px)`;
        ...
        getSelectedCount(c) { //当子组件把选中的数量传递给父组件的时候，把选中的值保存到data上
